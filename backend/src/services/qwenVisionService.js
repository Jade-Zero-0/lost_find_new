...
/**
 * 阿里通义千问 Qwen-VL 视觉识别服务（DashScope OpenAI 兼容接口）
 *
 * - 支持输入：图片路径 / 纯 Base64 / dataURL / Buffer
 * - 主模型 qwen-vl-plus 默认只尝试 1 次（QWEN_MAX_RETRIES=0），失败交由上层切换备用模型 qwen-vl-max
 * - 如需 429 退避重试，可设置 QWEN_MAX_RETRIES>0（重试次数）与 QWEN_RETRY_DELAY_MS（间隔毫秒）
 * - API Key 从环境变量 QWEN_API_KEY 读取（兼容旧变量 DASHSCOPE_API_KEY），绝不硬编码、绝不打印
 ...
 *  统一命名 QWEN_*，同时兼容 DASHSCOPE_*（QWEN_* 优先） */
export function getConfig() {
  return {
    // API Key 只从环境变量读取，绝不硬编码；QWEN_API_KEY 优先，DASHSCOPE_API_KEY 兼容
    apiKey: process.env.QWEN_API_KEY || process.env.DASHSCOPE_API_KEY || '',
    baseUrl: (process.env.QWEN_BASE_URL || process.env.DASHSCOPE_BASE_URL || DEFAULT_BASE_URL).replace(/\/+$/, ''),
    model: process.env.QWEN_MODEL || DEFAULT_MODEL,
    // 备用视觉模型（qwen-vl-max 高性能版：主模型 qwen-vl-plus 超时/失败时的第二优先）
    fallbackModel: process.env.QWEN_FALLBACK_MODEL || 'qwen-vl-max',
    timeoutMs: parseNonNegativeInt(process.env.QWEN_TIMEOUT_MS, parseNonNegativeInt(process.env.AI_TIMEOUT_MS, 20000)),
    maxRetries: parseNonNegativeInt(process.env.QWEN_MAX_RETRIES, 0),
    retryDelayMs: parseNonNegativeInt(process.env.QWEN_RETRY_DELAY_MS, 2000)
  };
}

/** 单次请求通义千问接口并映射错误码 */
async function callQwen(cfg, dataUrl, model) {
  ...
  if (!res.ok) {
    ...
    if (res.status === 401 || res.status === 403) {
      throw new AiError('通义千问 API Key 无效或无权限，请检查 QWEN_API_KEY 配置', AI_ERROR_CODES.AUTH, 502);
    }
    if (res.status === 429) {
...
/**
 * 通义千问 Qwen-VL 视觉识别（默认只尝试 1 次；可配置 429 退避重试）
...
export async function analyzeLostItemImage(image, { model, maxRetries } = {}) {
  const cfg = getConfig();
  if (!cfg.apiKey) {
    throw new AiError('未配置 QWEN_API_KEY（兼容旧变量 DASHSCOPE_API_KEY），请在环境变量中设置', AI_ERROR_CODES.MISSING_KEY, 502);
  }
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    try {
      return await callQwen(cfg, dataUrl, targetModel);
    } catch (err) {
      lastErr = err;
      const isRateLimit = err.code === AI_ERROR_CODES.RATE_LIMIT;
      if (attempt < maxAttempts && isRateLimit) {
        console.warn(`[ai] qwen 触发限流(429)，${cfg.retryDelayMs}ms 后进行第 ${attempt + 1} 次重试`);
